...
void iterate_step(const double* current, double* next, int size) {
    const int total = size * size;

    #pragma acc parallel loop collapse(2) present(current[0:total], next[0:total])
    for (int y = 1; y < size - 1; ++y) {
        for (int x = 1; x < size - 1; ++x) {
            ...
            const double laplace = -4.0 * current[position]
                                 ...
                                 + current[idx(y, x + 1, size)];
            next[position] = current[position] - TAU * laplace;
        }
    }
}

double max_delta(const double* current, const double* next, int size) {
    double error = 0.0;
    const int total = size * size;
    ...
    for (int y = 1; y < size - 1; ++y) {
        for (int x = 1; x < size - 1; ++x) {
            const int position = idx(y, x, size);
            error = std::max(error, std::abs(next[position] - current[position]));
        }
    }
    return error;
}
...
int main(int argc, char** argv) {
    ...
    {
        do {
            iterate_step(current_ptr, next_ptr, config.size);
            error = max_delta(current_ptr, next_ptr, config.size);
